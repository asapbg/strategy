...
<tr>
    <td valign="top">
        ...
        @php($label = __($buttonLabel) . ' ' . $a+1)
            @include('form_partials.textarea', ['name' => $nameSimple."[$a][text]", 'value' => old($nameSimple.'['.$a.']', array_key_exists($nameSimple, $state) ? data_get($state, "$nameSimple.$a.text") : ''), 'class' => $class_textarea ?? ''])
        @if($a > 0)

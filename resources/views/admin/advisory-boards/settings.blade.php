            ...
            <div class="card card-primary card-outline card-tabs">
                    ...
                    <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
                        @if(isset($sections) && sizeof($sections))
                            @foreach($sections as $s)
                                <li class="nav-item">
                                    <a class="nav-link @if($section == $s) active @endif" id="{{ $s }}-tab" href="{{ route('admin.advisory-boards.settings', ['section' => $s]) }}">{{ __('custom.settings.sections.'.$s) }}</a>
                                </li>
                            @endforeach
                        @endif
                    </ul>
                ...
                <div class="card-body">
                    @if(isset($settings) && $settings->count())
                        <form action="{{ route('admin.advisory-boards.settings.store') }}" method="post">
                            @csrf
                            @method('PUT')

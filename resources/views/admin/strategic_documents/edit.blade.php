                        ...
                        <ul class="nav nav-tabs" id="custom-tabs" role="tablist">
                            @foreach(\App\Http\Controllers\Admin\StrategicDocumentsController::SECTIONS as $s)
                                @if($item->id || $s != \App\Http\Controllers\Admin\StrategicDocumentsController::SECTION_FILES )
                                    <li class="nav-item">
                                        <a class="nav-link @if($section == $s) active @endif" id="{{ $s }}-tab"
                                           href="{{ route('admin.strategic_documents.edit', [$item, $s]) }}">{{ __('custom.strategic_documents.sections.'.$s) }}</a>
                                    </li>
                                @endif
                            @endforeach
                            @if($item->id)
                                <li class="nav-item">
                                    <button class="nav-link add_sd_document bg-success"
                                            data-url="{{ route('admin.strategic_documents.document.popup', ['sd' => $item]) }}">
                                        + {{ trans_choice('custom.strategic_documents.documents', 1) }}</button>
                                </li>
                            @endif

            ...
            <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-12">
                                <select id="strategic_document_level_id" name="strategic_document_level_id"
                                        ...
                                        @if($item->id && (!$item->strategic_document_level_id || $item->region_id)) disabled @endif
                                >
                                    <option value="">--</option>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-12">
                                <select id="policy_area_id" name="policy_area_id"
                                    ...
                                    <option value=""
                                            @if(old('policy_area_id', ($item->id ? $item->policy_area_id : '')) == '') selected @endif>
                                        ---
                                    </option>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-12">
                                <select id="region_id" name="region_id"
                                        class="form-control form-control-sm @error('region_id'){{ 'is-invalid' }}@enderror"
                                    ...
                                    @if(isset($regions) && sizeof($regions))
                                        @foreach($regions as $region)
                                            <option value="{{ $region->id }}"
                                                    @if(old('region_id', $item->region_id) == $region->id) selected @endif>
                                                {{ $region->name }}
                                            </option>
                                        @endforeach
                                    @endif
                ...
                <div class="row">
                    <div class="col-md-12" id="ekatte_area_div_id">
                        <div class="form-group">
                            ...
                            <div class="col-12">
                                ...
                                >
                                    <option value=""
                                            @if(old('ekatte_area_id', ($item->id ? $item->policy_area_id : '')) == '') selected @endif>
                                        ---
                                    </option>
                                    @if($ekateAreas && $ekateAreas->count())
                                        @foreach ($ekateAreas as $ekateArea)
                                            <option value="{{ $ekateArea->id }}"
                                                    @if(old('ekatte_area_id', ($item->id ? $item->policy_area_id : 0)) == $ekateArea->id) selected
                                                    @endif data-id="{{ $ekateArea->id }}">{{ $ekateArea->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class="col-12">
                                @if(isset($ekateMunicipalities))
                                    <select id="ekatte_municipality_id" name="ekatte_municipality_id"
                                        ...
                                        <option value=""
                                                @if(old('ekatte_municipality_id', ($item->id ? $item->policy_area_id : '')) == '') selected @endif>
                                            ---
                                        </option>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div class="col-12">
                                ...
                                >
                                    <option @if(old('accept_act_institution_type_id', $item->id ? $item->accept_act_institution_type_id : '') == '') selected @endif
                                        value=""
                                    >
                                        ---
                                    </option>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div class="col-12">
                                <select id="public_consultation_id" name="public_consultation_id" data-types2ajax="pc"
                                    ...
                                    @if($item->publicConsultation)
                                        <option value="{{ $item->publicConsultation->id }}"
                                                selected
                                                data-id="{{ $item->publicConsultation->id }}"> {{ $item->publicConsultation->reg_num }}
                                            / {{ $item->publicConsultation->title }} </option>
                                    @endif
                                </select>
                ...
                <div class="row">
                    <div class="col-md-12 d-none" id="pris-act">
                                ...
                                >
                                    ...
                                    @if(isset($legalActTypes) && $legalActTypes->count())
                                        @foreach($legalActTypes as $row)
                                            <option value="{{ $row->id }}"
                                                    @if($row->id == $item->pris?->legal_act_type_id) selected @endif>{{ $row->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <label class="col-sm-12 control-label" for="pris_act_id">Акт за приемане от раздел
                                    „Актове на МС“</label>
                        ...
                        <div class="form-group">
                            <label class="col-sm-12 control-label" for="document_date_accepted">
                                <i class="fas fa-info-circle text-info mr-2" data-toggle="tooltip"
                                   title="Когато документът е свързан с Акт за приемане от раздел „Актове на МС“, дата на приемане се взима автоматично от акта. Когато Дата на приемане остане празно, автоматично се взима днешна дата."></i>{{ __('custom.date_accepted') }}
    ...
    <div class="row">
        <div class="form-group row">
            <div class="col-md-6 col-md-offset-3">
                <button id="save" type="submit" name="active" value="{{ $item->active }}"
                        class="btn btn-success">{{ __('custom.save') }}</button>
                <button id="stayButton" type="submit" name="stay" value="1"
                ...
                @if($item->id)
                    ...
                    @else
                        <button id="save" type="submit" name="active" value="1"
                                class="btn btn-success">{{ __('custom.save_and_publish') }}</button>
                    @endif
                @else
                    <button id="save" type="submit" name="active" value="1"
                            class="btn btn-success">{{ __('custom.save_and_publish') }}</button>

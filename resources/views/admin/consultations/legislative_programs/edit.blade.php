...
{{--                                                <button type="submit" class="btn btn-sm btn-success">{{ __('custom.add') }}</button>--}}
                                ...
                                @if(isset($months) && sizeof($months))
                                    @foreach($months as $month)
                                        ...
                                        @php($hasData = 0)
                                        <tr class="bg-light">
                                            <td class="pt-4" style="border-bottom:1px solid #333;border-top:1px solid #333;" colspan="{{ $fullColspan }}"><strong>{{ trans_choice('custom.months', 1) }} {{ $month }}</strong></td>
                                        </tr>
                                        @if(isset($data) && $data)
                                            @foreach($data as $i => $row)
                                                @if($row->month == $month)
                                                    ...
                                                    <tr>
                                                        ...
                                                        @can('update', $item)
                                                            <td>
                                                                <a href="{{ route('admin.consultations.legislative_programs.remove_row', ['item' => $item, 'row' => $row->row_num]) }}"><i class="fas fa-trash text-danger"></i></a>
                                                            </td>
                                                        @endcan

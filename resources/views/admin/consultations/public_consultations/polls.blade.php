@if(auth()->user()->can('create', \App\Models\Poll::class))
    <div class="row mb-3">
        <div class="col-md-3">
            <a class="btn btn-sm btn-success" type="submit" href="{{ route('admin.polls.edit', ['id' => 0]).'?pc='.$item->id }}">
                <i class="fas fa-plus mr-2"></i>{{ __('custom.create').' '.__('custom.new_f').' '.trans_choice('custom.polls', 1) }}
            </a>
        </div>
    </div>
@endif
            ...
            @if($item->polls && $item->polls->count())
                @foreach($item->polls as $poll)
                    <tr>
                        ...
                        <td>
                            @can('update', $poll)
                                <a class="btn btn-sm btn-primary" href="{{ route('admin.polls.edit', ['id' => $poll->id]).'?pc='.$item->id }}">

...
<form action="{{ $storeRoute }}" method="post" name="form" id="form">
    ...
    <input type="hidden" name="nomenclature_level" id="nomenclature_level"
           value="@if(!$isAdmin || $item->id){{ $item->id ? $item->nomenclatureLevelLabel : $userInstitutionLevel }}@else{{ '0' }}@endif">
    <div class="row">
        @if($item->id && !$item->old_id)
            ...
            <div class="col-md-10">
                <div class="form-group">
                    <label
                        class="col-auto control-label">{{ trans_choice('site.public_consultation.responsible_institution', 1) }}
                        : </label> @if($item->responsibleInstitution)
                        <a class="text-primary"
                           href="{{ route('admin.strategic_documents.institutions.edit', $item->responsibleInstitution) }}"
                           target="_blank"><i
                                class="fas fa-link mr-1 fs-6"></i>{{ $item->responsibleInstitution->name }}</a>
                    @else
                        {{ '---' }}
                    @endif
                </div>
            ...
            @if($item->pris)
                <div class="col-12">
                    <div class="form-group">
                        <label class="col-auto control-label">{{ trans_choice('custom.pris_documents', 1) }}: </label>
                        <a class="text-primary" href="{{ route('admin.pris.edit', ['item' => $item->pris->id]) }}"
                           target="_blank"><i
                                class="fas fa-link mr-2"></i>{{ $item->pris->regNum.' ('.$item->pris->actType->name.')' }}
                        </a>
                    </div>
                </div>
            @endif
        @else
            @if(!$isAdmin)
                <div class="col-md-10">
                    <div class="form-group">
                        <label class="col-auto control-label">{{ trans_choice('custom.importers', 1) }}: </label>
                        {{ auth()->user() && auth()->user()->institution ? auth()->user()->institution->name : '---'}}
                    </div>
                </div>
                <div class="col-md-10">
                    <div class="form-group">
                        <label class="col-auto control-label">{{ __('custom.importer_address') }}: </label>
                        {{ auth()->user() && auth()->user()->institution ? ((auth()->user()->institution->settlement ? auth()->user()->institution->settlement->ime.', ' : '').auth()->user()->institution->address) : '---'}}
                    </div>
                </div>
            @else
                <div class="col-md-8">
                    <div class="form-group">
                        ...
                        <div class=" col-12 d-flex flex-row">
                            <div class="input-group">
                                <select
                                    class="form-control form-control-sm select2 @error('institution_id') is-invalid @enderror"
                                    ...
                                    @if(isset($institutions) && $institutions->count())
                                        @foreach($institutions as $option)
                                            <option value="{{ $option->value }}"
                                                    @if($option->value == old('institution_id', ($item->id ? ($item->institution_id ?? $item->importer_institution_id) : ''))) selected
                                                    @endif
                        ...
                        <div class="col-12">
                            @foreach($item->importerInstitution->links as $l)
                                @if(!$loop->first)
                                    <br>
                                @endif
                                <a href="{{ $l->link }}" target="_blank" class="main-color text-decoration-none"><i
                                        class="fas fa-regular fa-link  main-color me-1 fs-6"></i> {{ $l->title }}</a>
                            @endforeach
                        </div>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="col-12">
                    <select id="field_of_actions_id" name="field_of_actions_id" class="cl-child form-control form-control-sm select2 select2-no-clear
                            @error('field_of_actions_id'){{ 'is-invalid' }}@enderror"
                    >
                        <option value="">---</option>
        ...
        <div class="col-md-4">
            <div class="form-group">
                <label class="col-sm-12 control-label"
                       for="act_type_id">{{ trans_choice('validation.attributes.act_type_id', 1) }}<span
                        class="required">*</span></label>
                <div class="col-12">
                    <select id="act_type_id" name="act_type_id"
                            class="cl-child form-control form-control-sm select2 @error('act_type_id'){{ 'is-invalid' }}@enderror">
                        <option value="">---</option>
        ...
        <div class="col-md-4" id="law_section">
            <div class="form-group">
                ...
                <div class="col-12">
                    <select name="law_id"
                            class="form-control form-control-sm select2 @error('law_id'){{ 'is-invalid' }}@enderror">
        ...
        <div class="col-md-12 my-3" id="legislative_programs">
                    ...
                    <select name="legislative_program_id"
                        ...
                        @if(isset($legislativePrograms) && $legislativePrograms->count())
                            @foreach($legislativePrograms as $row)
                                <option value="{{ $row->id }}"
                                        @if(old('legislative_program_id', ($item->id ? $item->legislative_program_id : 0)) == $row->id) selected
                                        @endif
                                        data-id="{{ $row->id }}"
                                >{{ $row->name }}</option>
                            @endforeach
            ...
            <div class="form-group">
                ...
                <select id="legislative_program_row_id" name="legislative_program_row_id" data-types2ajax="lp_record_pc"
                    ...
                    @if(!old('legislative_program_row_id') && $item->legislative_program_row_id && $item->lpRow && $item->lpRow->parent)
                        <option
                            value="{{ $item->lpRow->id }}">{{ $item->lpRow->value }} {{ $item->lpRow->parent->recordPeriod }}</option>
                    @endif
                </select>
            ...
            <div class="form-group">
                <label class="col-sm-12 control-label" for="no_legislative_program">
                    <input type="checkbox" id="no_legislative_program" name="no_legislative_program"
                           @if(!old('no_legislative_program') && (($item->legislative_program_id && !$item->lpRow) && !$item->legislative_program_id )) checked
                           @endif
        ...
        <div class="col-md-12 my-3" id="operational_programs">
                    ...
                    <select name="operational_program_id"
                        ...
                        @if(isset($operationalPrograms) && $operationalPrograms->count())
                            @foreach($operationalPrograms as $row)
                                <option value="{{ $row->id }}"
                                        @if(old('operational_program_id', ($item->id ? $item->operational_program_id : 0)) == $row->id) selected
                                        @endif
                                        data-id="{{ $row->id }}"
                                >{{ $row->name }}</option>
                            @endforeach
            ...
            <div class="form-group">
                ...
                <select id="operational_program_row_id" name="operational_program_row_id"
                    ...
                    @if(!old('operational_program_row_id') && $item->operational_program_row_id && $item->opRow && $item->opRow->parent)
                        <option value="{{ $item->opRow->id }}"
                                selected="selected">{{ $item->opRow->value }} {{ $item->opRow->parent->recordPeriod }}</option>
                    @endif
                </select>
            ...
            <div class="form-group">
                <label class="col-sm-12 control-label" for="no_operational_program">
                    <input type="checkbox" id="no_operational_program" name="no_operational_program"
                           @if(!old('no_operational_program', null) && (($item->operational_program_id && !$item->opRow) && !$item->operational_program_id)) checked
                           @endif
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="form-group">
                <label class="col-sm-12 control-label" for="open_from">{{ __('validation.attributes.open_from') }} <span
                        class="required">*</span></label>
        ...
        <div class="col-md-4">
            <div class="form-group">
                <label class="col-sm-12 control-label" for="open_to">{{ __('validation.attributes.open_to') }} <span
                        class="required">*</span></label>
    ...
    <div class="row mb-2">
        <div class="form-group">
            <label class="col-md-12 control-label"
                   for="connected_pc">{{ __('custom.consultation_connections') }}</label>
            <div class="col-12">
                ...
                >
                    @if($item->consultations->count())
                        @foreach($item->consultations as $row)
                            <option value="{{ $row->id }}"
                                    selected>{{ $row->title.' ('.displayDate($row->open_from).' - '.displayDate($row->open_to).')' }}</option>
                        @endforeach
                    @endif
                </select>
                @error('connected_pc')
        ...
        @if($pcByDecree)
            <div class="form-group">
                <label class="col-sm-12 control-label"
                       for="connected_pc">{{ __('custom.connections_by_op_lp') }}</label>
                <div class="col-12">
                    {{--                @if($pcByOpLp->count())--}}
                    ...
                    @if($pcByDecree && $pcByDecree->count())
                        @foreach($pcByDecree as $row)
                            @if($row->id != $item->id)
                                @php($found = true)
                                <a class="d-block"
                                   href="{{ route('admin.consultations.public_consultations.edit', $row) }}"
                                   target="_blank">

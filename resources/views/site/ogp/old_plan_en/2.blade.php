...
{{--            </div>--}}
        ...
        <div class="row mb-3">
            @if(isset($planData) && isset($planData['content']) && sizeof($planData['content']))
                <div class="col-md-12">
                    <div class="accordion" id="accordionExampleContent">
                        @foreach($planData['content'] as $c)
                            <div class="accordion-item mb-2">
                                <h2 class="accordion-header" id="heading_content_{{ $loop->iteration }}">
                                    {{--                                aria-expanded="{{ $loop->first ? 'true' : 'false' }}"--}}
                                    <button class="accordion-button text-dark fs-18 fw-600" type="button"
                                            data-toggle="collapse" data-target="#collapse-content-{{ $loop->iteration }}"
                                            aria-controls="collapse-content-{{ $loop->iteration }}">
                                        {{ $c['title'] }}
                                    </button>
                                </h2>
                            </div>
                            <div id="collapse-content-{{ $loop->iteration }}" @class(["accordion-collapse", "collapse"])
                            aria-labelledby="heading_content_{{ $loop->iteration }}" data-parent="#accordionExampleContent" style="">

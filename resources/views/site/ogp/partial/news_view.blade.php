...
{{--        <span class="dark-blue"><i class="fas fa-sitemap me-1" title="{{ $publication->advCategory }}"></i> {{ $publication->advCategory }}</span>--}}
    ...
    <div class="col-md-4 text-end">
        @can('updateOgp', $publication)
            <a href="{{ route('admin.ogp.news.edit' , $publication) }}" target="_blank" class="btn btn-sm btn-primary main-color">
                <i class="fas fa-pen me-2 main-color"></i>{{ __('custom.edit') }}
            </a>
        @endcan
        @can('deleteOgp', $publication)
            <a href="javascript:;"
               class="btn btn-sm btn-danger"
               data-target="#modal-delete-resource"
               data-resource-id="{{ $publication->id }}"
               data-resource-name="{{ $publication->title }}"
               data-resource-delete-url="{{ route('admin.ogp.news.delete', $publication) }}"
               data-toggle="tooltip"

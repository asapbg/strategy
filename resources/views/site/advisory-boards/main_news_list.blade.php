    ...
    <div class="row">
        @foreach($items as $item)
            <div class="col-lg-4 mb-4">
                <div class="post-box">
                    <div class="post-img"><img src="{{ asset($item->id && $item->mainImg ? $item->mainImgAsset : $item->advDefaultImg) }}" class="img-fluid" alt=""></div>
                                ...
                                <div class="consult-item-header-edit">
                                    @can('deleteAdvBoard', $item)
                                        <a href="javascript:;"
                                           ...
                                           data-resource-delete-url="{{ route('admin.advisory-boards.news.delete', $item) }}"
                                           data-toggle="tooltip"
                                           title="{{ __('custom.delete') }}"><span class="d-none"></span>
                                        </a>
                                    @endcan
                                    @can('updateAdvBoard', $item)
                                        <a href="{{ route('admin.advisory-boards.news.edit', ['type' => $item->type, 'item' => $item->id]) }}" target="_blank">
                    ...
                    <a href="{{ route('advisory-boards.news.details', $item) }}" class="readmore stretched-link mt-1" target="_blank" title="{{ $item->title }}">{{ __('site.read_more') }} <i class="fas fa-long-arrow-right"></i></a>
                </div>
            </div>
        @endforeach
    </div>

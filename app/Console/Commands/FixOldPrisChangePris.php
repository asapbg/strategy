                                ...
                                if (sizeof($connection) == 5) {
                                    ...
                                    switch ($oldConnection) //type of Connection
                                    {
                                        ...
                                        case 'допълва':
                                            ...
                                            break;
                                        case 'поправка':
                                            $newConnection = PrisDocChangeTypeEnum::AMENDMENT->value;
                                            $prisId = $prisDocument->id;
                                            $changedPrisId = null;
                                            break;
                                        case 'поправка в':
...
//                                    dump($prisId, $changedPrisId, $newConnection, $oldConnection, $oldC);
    ...
    private function parseConnection($str)
    {
        ...
        if (sizeof($matches) != 5) {
            ...
            preg_match('/^(поправка в|изменя|изменен от|отменя|отменен от|допълва|допълнен от|виж) (РАЗП|разп|Разп|Решение|РЕШ|Реш|реш|ПРОТ|Прот|ПОСТ|Пост) ([\d*(?:\.\d+)?]{1,}) от ([\d]{4})+/', $str, $matches);
        }
        if (sizeof($matches) != 5) {

...
class LegalReason extends QueryFilter implements FilterContract
{
    ...
    public function handle($value, $filter = null): void
    {
        if (!empty($value)) {
            ...
            $where = "pris_translations.legal_reason $condition '%$value%'";
            if ($fullKeyword) {
                $where = "(";
                $where .= "pris_translations.legal_reason $condition '% $value %'";
                $where .= " OR pris_translations.legal_reason $condition '% $value'";
                $where .= " OR pris_translations.legal_reason $condition '$value %'";
                $where .= ")";
            }
            ...
            if (strstr($value, ",")) {
                ...
                if ($logicalAnd == "OR") {
                    foreach ($tags as $key => $tag) {
                        ...
                        if ($fullKeyword) {
                            $where .= "(";
                            $where .= "pris_translations.legal_reason $condition '% $tag %'";
                            $where .= " OR pris_translations.legal_reason $condition '% $tag'";
                            $where .= " OR pris_translations.legal_reason $condition '$tag %'";
                            $where .= ")";
                        } else {
                            $where .= "pris_translations.legal_reason $condition '%$tag%'";

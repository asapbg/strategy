...
//            ->whereStatus(LegislativeInitiativeStatusesEnum::STATUS_ACTIVE)
    ...
    public function getStrategicDocuments(Request $request)
    {
        ...
        $strategicDocuments = StrategicDocument::list($requestFilter, 'validFrom', 'desc', $paginate);
            ...
            foreach ($dbItems as $item) {
                switch ($item->type){
                    ...
                    case 'li':
                        ...
                        break;
                    case 'pris':
                        $items[] = ['url' => LaravelLocalization::localizeUrl(($item->in_archive ?? 0)
                            ? route('pris.archive.view', ['category' => Str::slug($item->slug), 'id' => $item->id])
                            : route('pris.view', ['category' => Str::slug($item->slug), 'id' => $item->id]), $item->locale), 'priority' => '0.8', 'lastmod' => $item->lastmod];
                        break;
                    case 'lp':

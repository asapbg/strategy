...
class ReportsController extends Controller
{
    ...
    const REPORTS_DIR = 'openData';
    public function apiReportSd(Request $request, string $type, bool $inFile = false){
        if($inFile) {
            switch ($type) {
                case 'standard':
                    $data = DB::select('
...
//                    'act_number' => '',
        ...
        return $this->responseReport($data ?? null, $inFile, 'sd', $type);
    }

    public function apiReportLegislativeInitiative(Request $request, string $type, bool $inFile = false){
        if($inFile) {
            switch ($type) {
                case 'standard':
        ...
        return $this->responseReport($data ?? null, $inFile, 'li', $type);
    }

    public function apiReportLp(Request $request, string $type, bool $inFile = false)
    {
        if($inFile) {
            switch ($type) {
                case 'standard':
        ...
        return $this->responseReport($data ?? null, $inFile, 'lp', $type);
    }

    public function apiReportOp(Request $request, string $type, bool $inFile = false)
    {
        if($inFile) {
            switch ($type) {
                case 'standard':
        ...
        return $this->responseReport($data, $inFile, 'op', $type);
    }
    public function apiReportPc(Request $request, string $type, bool $inFile = false)
    {
        if($inFile) {
            switch ($type) {
                case 'standard':
        ...
        return $this->responseReport($data ?? null, $inFile, 'pc', $type);
    }

    public function apiReportAdvBoards(Request $request, string $type, bool $inFile = false)
    {
        if($inFile) {
            switch ($type) {
                case 'standard':
        ...
        return $this->responseReport($data ?? null, $inFile, 'ab', $type);
    }
    public function apiReportPris(Request $request, string $type, bool $inFile = false){
        if($inFile) {
            switch ($type) {
                case 'standard':
                case 'archive':
...
//                    ->where('pris.id', '=', 131703)
        ...
        return $this->responseReport($data ?? null, $inFile, 'pris', $type);
    }

    public function apiReportImpactAssessments(Request $request, string $type, bool $inFile = false){
        if($inFile) {
            switch ($type) {
//            case 'standard':
...
//                break;
        ...
        return $this->responseReport($data ?? null, $inFile, 'ia', $type);
    }

    public function apiReportLibrary(Request $request, string $type = 'standard', bool $inFile = false){
        if($inFile) {
            switch ($type) {
                case 'standard':
...
//                    'author' => __('custom.author'),
        ...
        return $this->responseReport($data ?? null, $inFile, 'library', $type);
    }

    public function apiReportPolls(Request $request, string $type = 'standard', bool $inFile = false){
        if($inFile) {
            switch ($type) {
                case 'standard':
                    $data = DB::select(
...
//                array_unshift($data, $header);
        ...
        return $this->responseReport($data ?? null, $inFile, 'poll', $type);
    }

    public function apiOgp(Request $request, string $type = 'standard', bool $inFile = false){
        if($inFile) {
            switch ($type) {
                case 'full':
                    $data = DB::select('
        ...
        return $this->responseReport($data ?? null, $inFile, 'ogp', $type);
    }

    private function storeReport($data, $section, $type){
        Storage::disk('public_uploads')->put('openData/'.$section.'_'.$type.'.json', json_encode($data, JSON_UNESCAPED_UNICODE));
    }
    private function readReport($section, $type){
        if(Storage::disk('public_uploads')->exists('openData/'.$section.'_'.$type.'.json')){
            $result = Storage::disk('public_uploads')->get('openData/'.$section.'_'.$type.'.json');
            if(!empty($result)){
    ...
    private function responseReport($data, $inFile, $section, $type){
        if($inFile){
            $this->storeReport($data, $section, $type);
            return true;
        } else{
            return response()->json($this->readReport($section, $type), 200, ['Content-Type' => 'application/json;charset=UTF-8', 'Charset' => 'utf-8'], JSON_UNESCAPED_UNICODE);
        }
    }
}

            ...
            case 'subscriptions':
                $data = \DB::select('
                    select
                        ...
                        json_agg(json_build_object(\'id\', user_subscribes.id, \'subscribable_id\', user_subscribes.subscribable_id, \'search_filters\', user_subscribes.search_filters, \'is_subscribed\', user_subscribes.is_subscribed, \'title\', user_subscribes.title)) as subscriptions
...
//        dd($validated);
    ...
    public function subscriptionDelete(Request $request, int $id) {
        $user = auth()->user();
        $subscription = UserSubscribe::find($id);
        if(!$subscription){
            return back()->with('warning', __('messages.record_not_found'));
        ...
        $subscription->delete();
        return redirect(route('profile', ['tab' => 'subscriptions']))->with('success', __('messages.success_delete_subscribe'));
    }

    public function subscriptionStore(Request $request) {
        $user = auth()->user();
        $id = $request->input('subscription_id', null);
        $name = $request->input('subscription_name', '');

        if(!$id || empty($name)){
            return back()->with('error', __('site.store_subscription_missing_name'));
        }
        ...
        try {
            $subscription->update(['title' => $name]);
            return redirect(route('profile', ['tab' => 'subscriptions']))->with('success', __('messages.success_edit_subscribe'));
        } catch (\Exception $e) {
            Log::error($e);

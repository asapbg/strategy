...
class LegislativeInitiativePageController extends AdminController
{
    public function info(Request $request){
        $page = Page::with('files')->BySysName(Page::LEGISLATIVE_INITIATIVE_INFO)->first();
        if(!$page) {
            abort(404);
        }
        ...
        if($request->isMethod('put')){
            ...
            if($this->savePage($validator->validated(), $page)){
                return redirect(route('admin.legislative_initiatives.page.info') )
        ...
        return $this->editPage($page, 'admin.legislative_initiatives.page.info');
    }
